 ...
 * @param {number} depth 
 */
export const inference = (sources, parentNode, depth = 0) => {
 ...
 * @returns {line} line
 */
export const findIndex = (root, index) => {
    if (root.index === index) {
        return root;
    }

    for (const connection of root.connection) {
        if (connection.vline.index === index) {
            return connection.vline;
        } else {
            const find = findIndex(connection.vline, index);
            if (find) {
                return find;
            }
        }
